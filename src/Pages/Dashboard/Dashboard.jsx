...
const Dashboard = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      let res = await fetch(apiUrl('/api/bookings/my-bookings'), {
        headers: { 'Authorization': `Bearer ${token}` },
        credentials: 'include'
      });
      if (res.status === 401) {
        const refresh = await fetch(apiUrl('/api/auth/refresh-token'), {
          method: 'GET',
          credentials: 'include'
        });
        if (refresh.ok) {
          ...
          if (r?.accessToken) {
            ...
            token = r.accessToken;
            res = await fetch(apiUrl('/api/bookings/my-bookings'), {
              headers: { 'Authorization': `Bearer ${token}` },
              credentials: 'include'
            });
          }
  ...
  const handleCancel = async (id) => {
    ...
    const res = await fetch(apiUrl(`/api/bookings/cancel/${id}`), {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}` }
    });
